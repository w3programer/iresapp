...
class ColorViewVC: UIViewController {
    ...
    func brandPagination(numP: Int) {
        ...
        if Helper.checkToken() == true {
            url = URLs.categories+"/\(2)/"+"products?page=\(numP)"+"&token="+Helper.getUserToken()
        } else {
            url = URLs.categories+"/\(2)/"+"products"
        }
...
extension ColorViewVC: UICollectionViewDelegate, UICollectionViewDataSource , UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        return CGSize.init(width: width, height: self.collectionView.frame.size.height)
    }
